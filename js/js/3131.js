...
//		}
		...
		var img_file1 = img1.value;
		if(img_file1 != ""){
			if(fncGetLength(img_file1) > 32){
				fncAlert('画面上部イメージは半角32文字以内で入力してください',img1);
				return false;
			}
			if(!fnkChkDirName(img_file1)){
				fncAlert('画面上部イメージは半角英数-_で入力してください',img1);
				return false;
			}
			if(!img_file1.match(/\.(jpg|jpeg|gif|png)$/i)){
				fncAlert('画面上部イメージはjpg/jpeg/gif/pngにのみ対応しています',img1);
				return false;
			}
		}

		var img_file2 = img2.value;
		if(img_file2 != ""){
			if(fncGetLength(img_file2) > 32){
				fncAlert('画面下部イメージは半角32文字以内で入力してください',img2);
				return false;
			}
			if(!fnkChkDirName(img_file2)){
				fncAlert('画面下部イメージは半角英数-_で入力してください',img2);
				return false;
			}
			if(!img_file2.match(/\.(jpg|jpeg|gif|png)$/i)){
				fncAlert('画面下部イメージはjpg/jpeg/gif/pngにのみ対応しています',img2);
				return false;
			}
		}
...
//		}
		...
		if( (fncGetLength(upper_body.value)+fncGetLength(lower_body.value)) > 4000){
				fncAlert("テキスト（画面上部）とテキストテキスト（画面下部）合わせて全角2000文字以内で入力してください",upper_body);
				return false;
