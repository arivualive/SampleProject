...
//    '===== A100 =====
    ...
    if ($site_kbn === '2') {
        //    '===== B300 =====
        $text .= "=== Denbun Log B310 ===<br>".getHtmlEscapedString($session_id)."<br>";
        $sql = "Select to_char(REGIST_DT,'yyyy-mm-dd hh24:mi:ss') as RDT,HOST_DATA From B310Send_Tbl Where SESSION_ID=".getSqlValue($session_id)." Order by REGIST_DT";
    } else {
        //    '===== A310 =====
        $text .= "=== Denbun Log A310 ===<br>".getHtmlEscapedString($session_id)."<br>";

        //▼2008/08/01 #488 セッションログ日時表示(kdl yoshii)
        //$sql = "Select HOST_DATA From A300Send_Tbl Where SESSION_ID=".getSqlValue($session_id)." Order by REGIST_DT";
        $sql = "Select to_char(REGIST_DT,'yyyy-mm-dd hh24:mi:ss') as RDT,HOST_DATA From A310Send_Tbl Where SESSION_ID=".getSqlValue($session_id)." Order by REGIST_DT";
        //▲2008/08/01 #488 セッションログ日時表示(kdl yoshii)
    }
    // ▲モバイル対応 2009/03/17 kdl.ohyanagi
