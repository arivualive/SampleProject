...
if($SSK_DC_FLAG === false) {
	...
	if($TEST_FLAG === true ) {
		$TARGET_DB = 'テストDB(次世代版)'; 
			//▼R-#45290_【R03-0111-028】次世代システム_WEB管理ツール開発（DB接続）2021/05/18 jst-arivazhagan
			//ORACLE
			...
			//POSTGRES 
			$DB_HOST   = 'development-postgres-instance-1.csgi1mav6oeg.ap-northeast-1.rds.amazonaws.com';
			$DB_USER   = 'ssktooluser';
			$DB_PASS   = 'cstxcDrATWUJ';
			$DB_USER_V = 'ssktooluser';//View用
			$DB_PASS_V = 'cstxcDrATWUJ';//View用
			$DB_NAME   = 'postgres';
			$DB_NAME_V = 'postgres';
			//▲R-#45290_【R03-0111-028】次世代システム_WEB管理ツール開発（DB接続）2021/05/18 jst-arivazhagan
