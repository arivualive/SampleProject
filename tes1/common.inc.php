 ...
 */
// ▼R-#45290_【R03-0111-028】次世代システム_WEB管理ツール開発（SQLを次世代用に書き換え）2021/07/13 jst-arivazhagan
...
Function fncGetSskMember($con,$nmid,$arr_clum_list) {
	...
	$sql .= " FROM m_net_mbr ";
	$sql .= "JOIN m_offline_data ";
	$sql .= "ON  m_net_mbr.mbr_seq = m_offline_data.mbr_seq ";
	$sql .= "AND  m_net_mbr.net_mbr_cd = :nmid  ";

	$result = $con->prepare($sql);
	$result->bindParam(":nmid", $nmid, PDO::PARAM_STR );
	$result->execute();
	$rows = $result->rowCount();
	if ($rows > 0) {
		$row = $result->fetch(PDO::FETCH_ASSOC);
...
// ▲R-#45290_【R03-0111-028】次世代システム_WEB管理ツール開発（次世代のための画面改修）2021/07/13 jst-arivazhagan
...
Function fncGetAttributeId($con, $uid) {
	...
	//誕生日から年齢取得
	$age    = fncGetAge($member_data['era_kbn'], $member_data['birthday']);

	//地域コード
	$ken_cd = $member_data['pref_cd'];
	//性別
	$gender = $member_data['sex_kbn'];
	//最終購入経過日
	$last_buy_prog_day = fncGetLastBuyProgDay($member_data['last_buy_dt_1'], $member_data['last_buy_dt_2'], $member_data['last_buy_dt_3'], $member_data['last_buy_dt_4']);
	//初回購入経過日
	$first_buy_prog_day = fncGetFirstBuyProgDay($member_data['first_buy_dt_1'], $member_data['first_buy_dt_2'], $member_data['first_buy_dt_3'], $member_data['first_buy_dt_4']);
	//化粧品購入回数
	$purchace_cnt = $member_data['buy_cnt'];
	...
	*/
	...
	if ($last_buy_prog_day !== '') {
		...
		$result->bindParam(":last_buy_prog_day", $last_buy_prog_day, PDO::PARAM_STR );
		// ▲R-#45290_【R03-0111-028】次世代システム_WEB管理ツール開発（次世代のための画面改修）2021/07/13 jst-arivazhagan
	};
	if ($first_buy_prog_day !== '') {
		// ▼R-#45290_【R03-0111-028】次世代システム_WEB管理ツール開発（SQLを次世代用に書き換え）2021/07/13 jst-arivazhagan
		//ssk_db_bind_by_name($result,":first_buy_prog_day",$first_buy_prog_day,-1);
		$result->bindParam(":first_buy_prog_day", $first_buy_prog_day, PDO::PARAM_STR );
		// ▲R-#45290_【R03-0111-028】次世代システム_WEB管理ツール開発（次世代のための画面改修）2021/07/13 jst-arivazhagan
	};
	if ($purchace_cnt !== '') {
		// ▼R-#45290_【R03-0111-028】次世代システム_WEB管理ツール開発（SQLを次世代用に書き換え）2021/07/13 jst-arivazhagan
		//ssk_db_bind_by_name($result,":purchace_cnt",$purchace_cnt,-1);
		$result->bindParam(":purchace_cnt", $purchace_cnt, PDO::PARAM_STR );
		// ▲R-#45290_【R03-0111-028】次世代システム_WEB管理ツール開発（次世代のための画面改修）2021/07/13 jst-arivazhagan
	};
	//ssk_db_execute($result);
	$result->execute();
